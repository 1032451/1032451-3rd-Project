...
def run(logdir,hparams):
    with tf.summary.create_file_writer(logdir).as_default():
        ...
        tf.keras.metrics.Precision(name="precision",thresholds=0.47),tf.keras.metrics.Recall(thresholds=0.47,name='recall')])
        ...
        f1_val = 2.0*(precision_val*recall_val)/(precision_val+recall_val+(0.1)**6)
        tf.summary.scalar(F1_VAL, f1_val, step=1)
        
        y_pred = model.predict_proba(validation_features)
        ...
        plt.plot([0, 1], [0, 1], 'k--')
        plt.plot(fpr, tpr, label='Model ' +str(session_num)+' (F1-score = {:.3f})'.format(f1_val)+' (area = {:.3f})'.format(a_u_c),lw=0.75)
