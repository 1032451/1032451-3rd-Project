...
hparam_folder = 'logs/tuning_hyperparameters'

with tf.summary.create_file_writer(hparam_folder).as_default():
  hp.hparams_config(
    hparams=[HP_NUM_UNITS, HP_REGULARIZATION, HP_OPTIMIZER, HP_NUM_LAYERS, HP_LEARNING_RATE, HP_REGULARIZE_FUNCTION, HP_DROPOUT, HP_LAYER_WIDTH],
...
def run(logdir,hparams):
    with tf.summary.create_file_writer(logdir).as_default():
        ...
        #        mx=f1[i]
        #        opt_i=i
        #print(opt_i,pr[opt_i],re[opt_i],f1[opt_i])
...
for i in range(num_trials):
    ...
    hparams={
        HP_NUM_UNITS : HP_NUM_UNITS.domain.sample_uniform(),
        HP_REGULARIZATION : 10**(HP_REGULARIZATION.domain.sample_uniform()),
        HP_OPTIMIZER : HP_OPTIMIZER.domain.sample_uniform(),
        HP_NUM_LAYERS : HP_NUM_LAYERS.domain.sample_uniform(),
        HP_LEARNING_RATE : 10**(HP_LEARNING_RATE.domain.sample_uniform()),
        HP_REGULARIZE_FUNCTION : HP_REGULARIZE_FUNCTION.domain.sample_uniform(),
        HP_DROPOUT : HP_DROPOUT.domain.sample_uniform(),
        HP_LAYER_WIDTH : HP_LAYER_WIDTH.domain.sample_uniform(),
    }
